  ...
  <div class="min-h-screen bg-gray-50 p-6">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
        <div
          class="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0"
        >
          <!-- Search -->
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  >
                    {{
                      $t(
                        `orders.order_status.${order.order.status.toLowerCase()}`
                      )
                    }}
                  </span>
                </td>
            ...
            >
              <div>
                <p class="text-sm text-gray-700">
                  {{ $t("common.showing") }}
                  {{ (currentPage - 1) * pageSize + 1 }} {{ $t("common.to") }}
                  {{ Math.min(currentPage * pageSize, totalOrders) }}
                  {{ $t("common.of") }} {{ totalOrders }}
                  {{ $t("common.results") }}
                </p>
              </div>
              <div>
                  ...
                  <div class="relative">
                    ...
                    >
                      ...
                      >
                        ...
                        <div class="text-sm text-gray-500" v-if="client.phone">
                          {{ client.phone }}
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      <div>
                        ...
                        <div class="relative">
                          <input
                            ...
                            :placeholder="
                              $t('orders.search_product_placeholder')
                            "
                          ...
                          >
                            ...
                            >
                              ...
                              <div class="text-sm text-gray-500">
                                ...
                                <span v-if="product.sku" class="ml-2"
                      ...
                      <div class="flex items-center space-x-2">
                        <div class="text-sm font-medium">
                          {{
                            formatCurrency(
                              (item.qty || 0) * (item.unit_price || 0) * 100
                            )
                          }}
                        </div>
                        <button
            ...
            <div v-if="detailOrder">
              <div class="mb-4">
                <div class="text-sm font-semibold">
                  {{ $t("orders.order_number") }}:
                  {{ detailOrder.order.order_number }}
                </div>
                <div class="text-sm">
                  {{ $t("orders.client") }}: {{ detailOrder.client.name }}
                </div>
                <div class="text-sm">
                  {{ $t("orders.status") }}: {{ detailOrder.order.status }}
                </div>
              </div>
...
const filterProducts = (index: number, event: Event) => {
  ...
  if (query.length === 0) {
    ...
    filteredProducts.value[index] = allProducts.value
      .filter(
        (product) =>
